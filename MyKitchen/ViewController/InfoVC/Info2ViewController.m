...
//
	...
	if (isPad) {
		moreAppNames = @[@"myecard",@"everalbum",@"nsc"];
	}
	else{
		moreAppNames = @[@"myecard",@"instahat",@"everalbum",@"nsc"];
	}
	...
	scrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(0.28*width, (isPad?0.05:0.05)*height, 0.67*width, 0.2*height)];
...
//	scrollView.backgroundColor = [UIColor redColor];
	...
	for (int i = 0; i<[moreApps count]; i++) {
		...
		UIButton *b = [UIButton buttonWithFrame:CGRectMake(20+(sHorizontMargin+sButtonWidth)*i, sVMargin, sButtonWidth, sButtonWidth) title:nil image:nil target:self actcion:@selector(buttonClicked:)];
...
- (void)email{
	...
	NSDictionary *dict = [NSDictionary dictionaryWithObjectsAndKeys:@"TinyKitchen -- Amazing App for little kids!", @"subject",
						  SRecommendEmailBody,@"emailBody",
						  nil];
	
	[[ExportController sharedInstance] sendEmail:dict delegate:root];
}
- (void)supportEmail{
	
	
	NSDictionary *dict = [NSDictionary dictionaryWithObjectsAndKeys:@"Feedback for Tiny Kitchen", @"subject",
